...
extern void db_ioCount(BOOL bRead, int bytes);
  ...
  */
  ...
  if( id->locktype==NO_LOCK
   || (locktype==EXCLUSIVE_LOCK && id->locktype==RESERVED_LOCK)
	  ...
	  extern __declspec(thread) __int64 th_iCommandTimeout;

    while((res = LockFile(id->h, PENDING_BYTE, 0, 1, 0))==0)
	{
		__int64 ft;

		GetSystemTimeAsFileTime((FILETIME*)&ft);

		if(th_iCommandTimeout < 32767)
			th_iCommandTimeout = ft + th_iCommandTimeout * 10000;
		else if(ft > th_iCommandTimeout)break;
